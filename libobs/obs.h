...
struct login_data {
	query_param_t username;
	query_param_t password;
};
typedef struct login_data login_data_t;

struct captcha_data {
	query_param_t sid;
	query_param_t key;
};
typedef struct captcha_data captcha_data_t;
...
struct subcategories_arr {
	struct subcategory *array;
	size_t size;
};
typedef struct subcategories_arr subcategories_arr_t;
...
struct categories_arr {
	struct category *array;
	size_t size;
};
typedef struct categories_arr categories_arr_t;
...
EXPORT int get_loginstatus(json_t *root);
EXPORT size_t write_query_data(char *buffer, size_t size, size_t nmemb,
	void *userdata);
EXPORT char *auth_query(unsigned nmemb, ...);
EXPORT char *api_query(const char *method, unsigned nmemb, ...);
EXPORT size_t get_captchaimg(const char *url, char **img_p);
EXPORT json_t *simple_login(login_data_t *login);
EXPORT json_t *login_w_captcha(login_data_t *login, captcha_data_t *captcha);
EXPORT json_t *login_w_code(login_data_t *login, query_param_t code);

EXPORT void get_userdata(const char *access_token, char **fullname_p);
EXPORT long long get_groupsdata(const char *access_token,
	char ***groups_names_p, char ***groups_ids_p);
EXPORT categories_arr_t *get_categories(const char *access_token_str);
EXPORT long long init_vk_stream(obs_service_t *service,
	const char *output_name,
	const char *output_id,
	const char *access_token);
EXPORT void stop_vk_stream(const char *output_id, const char *access_token);

EXPORT bool enter_vk(void);

const char *auth_url = "https://oauth.vk.com/token?grant_type=password&client_id=5554299&client_secret=DEADBEEFDEADBEEF&v=5.53&2fa_supported=1&scope=270352%s";
...
/** Get a scene by its name. Custom function. */
EXPORT obs_scene_t *get_scene_by_name(const char *name);
